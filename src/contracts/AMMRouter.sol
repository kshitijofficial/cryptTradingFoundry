...
contract AMMRouter is IAMMRouter {
    ...
    function getAmountsIn(
        ...
        for (uint256 i = path.length - 1; i > 0; i--) {
            ...
            amounts[i - 1] = AMMLibrary.getAmountIn(
                amounts[i],
                reserveIn,
                reserveOut
            );
    ...
    function _swap(
        ...
        for (uint256 i = 0; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ? AMMLibrary.pairFor(factory, output, path[i + 2], initCodeHash)
                : _to;

            ITokenPair(AMMLibrary.pairFor(factory, input, output, initCodeHash))
    ...
    function swapExactTokensForTokens(
        ...
        AMMLibrary.safeTransferFrom(
            path[0],
            msg.sender,
            AMMLibrary.pairFor(factory, path[0], path[1], initCodeHash),
            amounts[0]
        );
